...
export const SendMoney = () => {
  ...
  const verifyAndProgress = async (id: string, amt?: string | null) => {
    ...
    try {
        ...
        if (found) {
            ...
            setError('Destinatario no encontrado. Verifique el email o número de cuenta.');
        }
    } catch (e) {
        setError('Error buscando al destinatario.');
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const handleVerifyRecipient = async (e: React.FormEvent) => {
    ...
    if (recipient.length < 3) {
      setError('Por favor ingrese un email o número de cuenta válido');
      return;
    }
  ...
  const initiateTransfer = () => {
      if (!user.transactionPin) {
          setError("Por favor configure un PIN de Transacción en Ajustes antes de enviar dinero.");
          return;
      }
  ...
  const handleFinalTransfer = async (e: React.FormEvent) => {
    ...
    try {
        const val = parseFloat(amount);
        const success = await sendMoney(val, recipient, note || 'Transferencia', pin);
        
        if (success) {
            setShowPinModal(false);
            setStep(4);
        } else {
            setError('La transacción falló. Error interno.');
        }
    } catch (e: any) {
        setError(e.message || "La transacción falló");
    } finally {
        setIsLoading(false);
    }
  };
    ...
    <div className="max-w-xl mx-auto py-8 px-4 animate-fade-in">
      ...
      {step === 1 && (
        <div className="space-y-6">
             <Card className="p-6">
                <form onSubmit={handleVerifyRecipient} className="space-y-4">
                    <Input 
                        ...
                        autoFocus
                    />
                    <Button type="submit" className="w-full" isLoading={isLoading}>Continuar</Button>
                </form>
             </Card>

             {recentRecipients.length > 0 && (
                 <div>
                     <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 px-1">Contactos Recientes</h3>
             ...
             <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl flex gap-3 border border-blue-100 dark:border-blue-900/30">
                 ...
                 <div>
                     <h4 className="font-black text-[10px] uppercase tracking-wider text-blue-900 dark:text-blue-300">Transferencia Segura</h4>
                     <p className="text-[10px] font-medium text-blue-700 dark:text-blue-400 mt-1">Las transferencias directas entre bancos son instantáneas y protegidas.</p>
                 </div>
             </div>
        </div>
      )}
            ...
            <form onSubmit={handleAmountSubmit} className="space-y-6">
                <div className="relative">
                    ...
                    <p className="text-center text-[10px] font-black uppercase text-gray-400 mt-2 tracking-widest">
                        Disponible: <span className="text-brand-600">${user.balance.toLocaleString()}</span>
                        <button type="button" onClick={setMaxAmount} className="ml-2 text-brand-600 hover:underline">USAR MAX</button>
                    </p>
                </div>
                ...
                <Button type="submit" className="w-full h-12 text-sm uppercase font-black tracking-widest bg-brand-700">Confirmar Monto</Button>
            </form>
        </Card>
      )}
      ...
      {step === 3 && (
          <div className="space-y-6">
              <Card className="p-6 overflow-hidden relative">
                  ...
                  <h3 className="text-gray-400 text-[9px] font-black uppercase tracking-[0.2em] mb-6">Revisar Detalles</h3>
                  
                  <div className="flex items-center justify-between mb-8">
                      <div className="text-center">
                          <img src={user.avatarUrl} alt="" className="w-10 h-10 rounded-full mx-auto mb-2 border-2 border-brand-50" />
                          <p className="text-[10px] font-black uppercase text-gray-900 dark:text-white">Tú</p>
                      </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-5 space-y-4">
                      <div className="flex justify-between items-center">
                          <span className="text-[10px] font-black text-gray-400 uppercase">Monto</span>
                          <span className="font-black text-lg">${parseFloat(amount).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                          <span className="text-[10px] font-black text-gray-400 uppercase">Comisión</span>
                          <span className="text-[10px] font-black text-emerald-500 uppercase">GRATIS</span>
                      </div>
                      {note && (
                         <div className="flex justify-between items-center border-t border-gray-100 dark:border-gray-800 pt-3">
                            <span className="text-[10px] font-black text-gray-400 uppercase">Nota</span>
                      ...
                      <div className="border-t-2 border-brand-100 dark:border-brand-800 pt-3 flex justify-between items-center">
                          <span className="text-[10px] font-black text-brand-900 dark:text-white uppercase tracking-wider">Cargo Total</span>
      ...
      {step === 4 && (
        <div className="text-center py-10 animate-scale-up">
            ...
            <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight mb-2">¡Transferencia Enviada!</h2>
            <p className="text-[11px] font-medium text-gray-500 mb-8 max-w-xs mx-auto">
                Has enviado <span className="text-brand-700 dark:text-brand-400 font-black">${parseFloat(amount).toFixed(2)}</span> a <span className="font-black uppercase">{selectedContact?.name}</span>.
            </p>
      ...
      <Modal isOpen={showPinModal} onClose={() => setShowPinModal(false)} title="Control de Seguridad">
          <form onSubmit={handleFinalTransfer}>
              <div className="text-center mb-6">
                ...
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Ingrese PIN de 4 dígitos para confirmar</p>
              </div>
              <Input 
                type="password"
                placeholder="0000"
              ...
              <div className="mt-6 flex gap-3">
                  <Button type="button" variant="ghost" onClick={() => setShowPinModal(false)} className="flex-1 uppercase text-[10px] font-black tracking-widest">Cancelar</Button>
                  <Button type="submit" isLoading={isLoading} disabled={pin.length !== 4} className="flex-1 bg-brand-700 uppercase text-[10px] font-black tracking-widest">Autorizar</Button>
              </div>
          </form>
      </Modal>
    </div>
