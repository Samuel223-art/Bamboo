      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex-1">
              ...
              <div className="p-3 bg-brand-50 dark:bg-brand-800 rounded-xl border border-brand-100 dark:border-brand-700 mb-3">
                  <p className="text-[8px] font-black text-brand-400 uppercase mb-1 tracking-widest">Tarea</p>
              ...
              <div className="flex items-center gap-3 text-[9px] font-black text-brand-400 uppercase tracking-wider">
                  ...
                  <span className="flex items-center gap-1">
                      ...
                      {isIncoming ? deal.creatorName : (deal.counterpartyName || 'Socio')}
                  </span>
              </div>
          </div>
          ...
          <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-4 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-brand-100 dark:border-brand-800 md:pl-6">
              <div className="text-left md:text-right">
                  <p className="text-[8px] font-black text-brand-400 uppercase tracking-widest">Valor</p>
                  <p className="text-xl font-black text-brand-900 dark:text-white">${deal.amount.toLocaleString()}</p>
              </div>
              <div className="flex flex-col gap-2 min-w-[120px]">
                  {!isIncoming && deal.status === 'active' && <Button size="sm" onClick={() => onRelease(deal.id)} className="w-full bg-brand-700 rounded-lg py-2">Liberar Fondos</Button>}
                  {isIncoming && deal.status === 'pending_acceptance' && <Button size="sm" onClick={() => onAccept(deal.id)} className="w-full bg-emerald-600 rounded-lg py-2">Aceptar Trato</Button>}
                  {deal.status === 'active' && <Button size="sm" variant="ghost" className="text-red-500 text-[10px] py-1">Disputar</Button>}
              </div>
          </div>
      </div>
  </Card>
...
export const Broker = () => {
  ...
  const handleCreateDeal = async (e: React.FormEvent) => {
    ...
    if (success) {
        ...
        setError('Falló la verificación.');
    }
  };
    ...
    <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
                ...
                <p className="text-xs text-brand-600 dark:text-brand-400 font-bold uppercase tracking-wide">Comercio P2P Seguro</p>
            </div>
            <Button size="sm" onClick={() => setIsModalOpen(true)} className="rounded-lg bg-brand-700 px-6">
                <Plus className="w-3.5 h-3.5 mr-1" /> Nuevo Trato
            </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-5 bg-brand-800 text-white border-none rounded-2xl shadow-lg relative overflow-hidden">
                <div className="relative z-10">
                    <p className="text-[8px] font-black text-brand-200 uppercase tracking-widest mb-1 flex items-center gap-1.5"><Shield size={10} /> Capital Bloqueado</p>
            ...
            <Card className="p-5 rounded-2xl border-brand-100 shadow-sm">
                <p className="text-[8px] font-black text-brand-400 uppercase tracking-widest mb-1">Requiere Atención</p>
            ...
            <Card className="p-5 rounded-2xl border-brand-100 shadow-sm">
                <p className="text-[8px] font-black text-brand-400 uppercase tracking-widest mb-1">Tratos Exitosos</p>
                <p className="text-2xl font-black text-brand-900 dark:text-white">
        ...
        <div className="space-y-6">
            {deals.filter(d => d.counterpartyId === user?.id).length > 0 && (
                <div>
                    <h2 className="text-sm font-black text-brand-900 dark:text-white mb-3 uppercase tracking-widest flex items-center gap-1.5">
                        <ArrowLeft size={14} className="text-emerald-500" /> Entrantes
                    </h2>
            ...
            <div>
                <h2 className="text-sm font-black text-brand-900 dark:text-white mb-3 uppercase tracking-widest flex items-center gap-1.5">
                    <ArrowRight size={14} className="text-brand-600" /> Salientes
                </h2>
                {deals.filter(d => d.creatorId === user?.id).length > 0 ? (
                    ...
                    <div className="text-center py-10 bg-brand-50/50 dark:bg-brand-900/10 rounded-2xl border-2 border-dashed border-brand-100 dark:border-brand-800">
                        <p className="text-[10px] font-black text-brand-400 uppercase">Bóveda Vacía</p>
                    </div>
                )}
            </div>
        </div>

        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Nuevo Escrow">
            <form onSubmit={handleCreateDeal} className="space-y-4">
                <Input label="Título" placeholder="Servicio o Artículo" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} required />
                <Input label="Email del Socio" type="email" placeholder="socio@dominio.com" value={formData.counterpartyEmail} onChange={e => setFormData({...formData, counterpartyEmail: e.target.value})} required />
                <Input label="Monto ($)" type="number" placeholder="0.00" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} required />
                <div>
                    <label className="block text-[10px] font-black text-brand-400 uppercase tracking-widest mb-1.5">Tareas / Entregables</label>
                    <textarea className="w-full bg-brand-50 dark:bg-brand-800 border border-brand-100 dark:border-brand-700 rounded-lg py-2 px-3 text-xs font-bold focus:ring-1 focus:ring-brand-500" rows={2} placeholder="Detalles de lo que se debe entregar..." value={formData.task} onChange={e => setFormData({...formData, task: e.target.value})} />
                </div>
                {error && <p className="text-red-500 text-[10px] font-bold bg-red-50 p-2 rounded-lg">{error}</p>}
                <div className="flex justify-end pt-2 gap-2">
                    <Button type="submit" size="sm" className="rounded-lg px-6 bg-brand-700 font-bold">Lanzar Trato</Button>
                </div>
            </form>
        </Modal>
    </div>
