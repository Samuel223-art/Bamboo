...
const ActivityBar: React.FC<{ height: string; day: string; active?: boolean; income: number; expense: number }> = ({ height, day, active, income, expense }) => (
  <div className="flex flex-col items-center gap-1 flex-1 group relative">
    <div className="absolute bottom-full mb-1 hidden group-hover:block bg-brand-900 text-white text-[8px] rounded p-1.5 z-10 whitespace-nowrap shadow-xl">
        <div className="text-brand-300">In: +${income.toLocaleString()}</div>
        <div className="text-red-300">Out: -${expense.toLocaleString()}</div>
    </div>
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-black text-brand-900 dark:text-white tracking-tight">Overview</h1>
          <p className="text-xs text-brand-600 dark:text-brand-400 font-bold uppercase tracking-wide">Hello, {user.name.split(' ')[0]}</p>
        </div>
          ...
          <div className="relative z-10 flex flex-col h-full justify-between min-h-[160px]">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-brand-200 font-black uppercase text-[8px] tracking-[0.2em]">Total Balance</p>
                <span className="bg-emerald-500/20 px-1.5 py-0.5 rounded-full text-[8px] font-black text-emerald-400 border border-emerald-400/20">GROWING</span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-white/5 backdrop-blur-md rounded-xl p-3 border border-white/10 hover:bg-white/10 transition-all cursor-pointer group" onClick={copyToClipboard}>
                <p className="text-[8px] font-black uppercase text-brand-300 mb-1 tracking-wider">Acct No.</p>
              ...
              <div className="flex gap-2">
                 ...
                 <div className="flex-1 bg-emerald-500/10 backdrop-blur-md rounded-xl p-3 border border-emerald-400/10">
                    <p className="text-[8px] font-black uppercase text-emerald-400 mb-0.5 text-center">Deals</p>
        ...
        <div className="space-y-4">
           <Card className="p-4 border-brand-100 rounded-2xl shadow-md">
              <div className="flex items-center justify-between mb-4">
                  <h3 className="font-black text-brand-900 dark:text-white text-sm uppercase tracking-wider">Quick Send</h3>
              </div>
              <div className="flex gap-3 mb-4 overflow-x-auto pb-1 scrollbar-hide">
                  <button onClick={() => navigate('/send')} className="flex flex-col items-center min-w-[50px] gap-1 group">
                     ...
                     <span className="text-[8px] text-brand-600 font-black uppercase">Add</span>
                  </button>
              ...
              <div className="space-y-2.5">
                  <Input placeholder="Email or Account" value={quickRecipient} onChange={e => setQuickRecipient(e.target.value)} className="rounded-lg border-brand-100" />
                  ...
                  <Button size="sm" className="w-full rounded-lg py-2.5 bg-brand-700 font-bold" onClick={handleQuickSend}>Send Funds</Button>
              </div>
           </Card>
          ...
          <Card className="p-4 border-brand-100 rounded-2xl">
            <h3 className="font-black text-brand-900 dark:text-white text-xs uppercase tracking-widest mb-3 flex items-center gap-2">
              <TrendingUp size={14} className="text-emerald-500" /> Market
            </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="p-5 border-brand-100 rounded-2xl shadow-md">
          <div className="flex items-center justify-between mb-6">
             <div>
               <h3 className="text-sm font-black text-brand-900 dark:text-white uppercase tracking-widest">Activity</h3>
               <p className="text-[9px] font-bold text-brand-400 uppercase tracking-wide">Last 7 Days</p>
             </div>
          </div>
          <div className="flex items-end justify-between gap-2.5 h-32 px-1">
             {activityData.length > 0 ? (
                 ...
                 <div className="w-full h-full flex flex-col items-center justify-center text-brand-300">
                     ...
                     <p className="text-[8px] font-black uppercase">Dry Season</p>
                 </div>
             )}
          </div>
        </Card>
        ...
        <Card className="lg:col-span-2 p-5 border-brand-100 rounded-2xl shadow-md">
             <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-black text-brand-900 dark:text-white uppercase tracking-widest">Transactions</h3>
                <Link to="/transactions" className="text-[10px] font-black text-brand-600 hover:underline uppercase">History</Link>
             </div>
             <div className="space-y-1">
                {user.id && <p className="text-center py-6 text-brand-300 text-[10px] font-bold uppercase italic tracking-widest">Awaiting Input...</p>}
             </div>
        </Card>
      </div>
    </div>
